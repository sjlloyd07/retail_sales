...
-- create temp table with all stock codes and final prices
DROP TABLE IF EXISTS double_temp;
CREATE TEMP TABLE new_price AS
	WITH double_price AS ( -- creates table with double price row numbers
		SELECT 
			DISTINCT stock_code, description, unit_price,
			ROW_NUMBER() OVER(PARTITION BY stock_code, description ORDER BY unit_price) AS rn
		FROM sales_temp
		GROUP BY stock_code, unit_price, description
		ORDER BY stock_code
		)

	SELECT stock_code, description, unit_price -- returns table with every item priced
	FROM double_price
	WHERE rn = 1 


-- update sales_temp with new prices
UPDATE sales_temp 
SET unit_price = (
	SELECT np.unit_price
	FROM new_price np
	WHERE sales_temp.stock_code = np.stock_code);
